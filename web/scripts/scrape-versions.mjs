...
// Issue:  #179   Run:  node web/scripts/scrape-versions.mjs
...
async function main() {
  ...
  for (const c of COMPONENTS) {
    ...
    if (c.strategy === 'static') {
      components.push({ ...base, version: c.version, build: c.build, releaseDate: null, sourceUrl: c.sourceUrl, patched: false });
      continue;
    }
    try {
      const r = c.strategy === 'kb' ? await scrapeKb(c) : await scrapeTechdocs(c);
