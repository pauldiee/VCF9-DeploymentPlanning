...
function day2Epic(sel: Selection): Epic {
  ...
  stories.push({
    ...
    tasks: [
      c.identityBroker
        ? `Now that all components exist, do the full CA-signed certificate replacement across the whole fleet. Select components in bulk and work through Generate CSRs -> sign -> Replace With Configured CA Certificate, but run it in staggered batches: each rotation triggers automated retrust across dependent components, and the UI makes you acknowledge that one batch must finish before the next starts — budget settling time between batches. The replace step reuses the LAST generated CSRs, so regenerate if a SAN or FQDN changed. Then complete fleet SSO via the VCF Identity Broker (configuration, not deployment — the broker has been running since bring-up; this is the recommended identity path, deferred from E6 6.3 — prep the AD/LDAP identity source and its gotchas first: see prerequisites.md, Identity source for the VCF Identity Broker), and apply licensing across the fleet (via VCF Operations). TechDocs: configure a CA — ${TECHDOCS.configureCa} ; configure an identity provider — ${TECHDOCS.identityProvider}`
        : `Now that all components exist, do the full CA-signed certificate replacement across the whole fleet. Select components in bulk and work through Generate CSRs -> sign -> Replace With Configured CA Certificate, but run it in staggered batches: each rotation triggers automated retrust across dependent components, and the UI makes you acknowledge that one batch must finish before the next starts — budget settling time between batches. The replace step reuses the LAST generated CSRs, so regenerate if a SAN or FQDN changed. Apply licensing across the fleet (via VCF Operations). Identity stays on the direct vCenter SSO AD/LDAP binding from E6 6.3 (broker-based fleet SSO is not in this scope — the bring-up Identity Broker stays unconfigured). TechDocs: configure a CA — ${TECHDOCS.configureCa}`,
    ],
    acceptance: c.identityBroker
      ? 'Every fleet endpoint presents a CA-signed cert with no trust warnings, with each rotation batch completed and settled before the next was started; AD/LDAP SSO via the Identity Broker works; licensing applied.'
      : 'Every fleet endpoint presents a CA-signed cert with no trust warnings, with each rotation batch completed and settled before the next was started; licensing applied.',
  });
