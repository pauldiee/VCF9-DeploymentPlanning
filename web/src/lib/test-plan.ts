...
const TP0: Entry[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'Confirm the planned groups exist: `Get-ADGroup -Filter "Name -like \'<vcf-group-prefix>*\'" | Select Name`. Then confirm each holds the expected members: `Get-ADGroupMember <group-name> | Select -Expand SamAccountName`.',
  ...
  },
  {
    ...
    story: '4.4',
    steps: [
      'From the jump/bastion host, reach each planned appliance address: `@("<esxi-1>","<esxi-2>","<vcenter>","<sddc-manager>") | ForEach-Object { Test-NetConnection $_ -Port 443 }`.',
      ...
      'Check the BMCs answer across the board: `@("<bmc-1>","<bmc-2>") | ForEach-Object { Test-NetConnection $_ -Port 443 }`.',
...
const TP2: Entry[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'Or from a template with PowerCLI: `New-VM -Name TestVM-A -Template <template> -ResourcePool <cluster> -Datastore <datastore>`, then the same for `TestVM-B`.',
  ...
  },
  {
    ...
    steps: [
      ...
      'Watch the VM restart elsewhere: `while ($true) { Get-VM TestVM-A | Select Name,VMHost,PowerState; Start-Sleep 15 }` (Ctrl+C to stop once it reports back on the new host).',
...
const TP3: Entry[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'Confirm workloads on the surviving AZ keep running, and workloads from the isolated AZ restart there: `while ($true) { Get-VM TestVM-A,TestVM-B | Select Name,VMHost,PowerState; Start-Sleep 15 }` (Ctrl+C to stop once it reports back on the surviving AZ).',
...
const TP4: Entry[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'Confirm each vCenter carries its primary licence and any add-ons: `Get-VIServer | Select Name` then check each in **vCenter → Administration → Licensing → Assets**.',
