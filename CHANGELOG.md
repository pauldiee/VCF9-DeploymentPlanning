...
- **docs/22: clarify AZ2 TEP pool reuse is officially supported at the API
  ...
  own [Clusters API docs](https://developer.broadcom.com/xapis/vmware-cloud-foundation-api/latest/clusters/#_usecase_stretchCluster)
  document referencing an
...
- **`CLAUDE.md` / `tools/Get-VCFCredentials.ps1`: generalize where
  ...
  reference is now generic "separate per-engagement storage" language – a
  public repo shouldn't spell out local filesystem layout.
  `Get-VCFCredentials.ps1` bumped to v1.2.1.
...
- **docs/19: add a *Setting it up* how-to to the Split DNS section, + Fojta's
  ...
  Now: the external IP block splits into an **internal range** (node IPs,
  service-runtime, Avi SE ingress / VS pool) and an **external/public range**
  (the FQDN / VIP only, design example `172.16.49.0/24` vs `10.200.0.0/24`); the
  **VCFA deploy JSON is unchanged**; the node pool + ingress IPs come from the
  internal range and the Avi VS VIP from the external range; and the two DNS
  views spelled out as A-record recipes (internal → node pool + service-runtime
  FQDN; external → the VS VIP, no service-runtime). Plus the *Result* end state.
  Added a **[field caveat]** from Tom Fojta's *Load Balancing VCF Automation
  with Avi*: a VCFA install's FQDN can't be changed post-install, and without
  the split-DNS entry the external VIP *"will start redirecting subsequent calls
  to the internal VIP"* — so it is closer to required than optional. Fojta's
  post added to *References* (also flags an HTTP/2 / VKS-gRPC requirement with an
  Avi 32.1.1 HEAD-request workaround — a `docs/14` follow-up).
...
- **Field-verified (#240): License Hub's endpoint-onboarding Certificate
  field wants the full chain, pasted in one go.** Onboarding a VCF-managed
  Avi Controller means the leaf certificate and the CA certificate both go
  into the same paste box in a single action — no separate leaf/CA fields —
  and since the controller's cert is auto-generated by VCF Operations, it
  has to be extracted from the controller first rather than already being
...
- **Avi: reserve four IPs, type three** (#209). Settled after a wrong turn in
  ...
  wizard takes **Node 1/2/3 IP Address**, then under *"Enter the VIP for cluster
  access"* offers only **Cluster FQDN** and **Cluster Name**. **The VIP address
  is never typed** — it is reached through the FQDN, which is exactly why that A
  record must resolve **before** the deploy. The VIP stays in the IP plan: a
  real address to reserve and exclude from DHCP, just not a wizard input.
  `prerequisites.md`, `01-network-dns-plan.md` and intake `E16` all say this.
- **License Hub instance passwords are `admin` and `audit` — those two only**
  (#207), confirmed by scrolling the dialog; the earlier "there may be more"
  hedge is removed. Two instance passwords on top of the installer's four.

## v2.4.8 — 2026-07-22 — **withdrawn, see v2.4.9**
- ~~Correction to v2.4.7: the Avi wizard does have a VIP field~~ — **this entry
  was wrong**. v2.4.7's original claim was correct.

## v2.4.7 — 2026-07-22
- **The Avi cluster FQDN must resolve BEFORE the deploy** (#209). Stands as
  written — the v2.4.8 challenge to it was withdrawn; see v2.4.9.
