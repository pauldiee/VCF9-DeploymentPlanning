<#
    ============================================================
    MIRRORED COPY -- NOT AUTHORED BY THIS PROJECT
    ============================================================

    fleet_lcm_deploy_vcf_automation_to_different_network.ps1

    Author    : William Lam
    Source    : https://github.com/lamw/vmware-scripts
                powershell/fleet_lcm_deploy_vcf_automation_to_different_network.ps1
    Blog post : https://williamlam.com/2026/06/vcf-9-1-deploying-vcf-automation-vcfa-to-non-management-network.html
    Licence   : BSD 2-Clause -- see LICENSE-lamw-vmware-scripts.txt beside this file

    Copyright (c) 2022, William Lam. All rights reserved.

    Redistributed unmodified under the BSD 2-Clause Licence. Only this header
    was added, to carry the copyright notice and provenance with the file.

    Mirrored : 2026-07-21
    SHA256 of the pristine upstream file at mirror time:
      E8B7F2C2CF39F9E3F2A494FEE560631321D20304718642DAACA1EA24B584B8D3

    ALWAYS PREFER THE UPSTREAM COPY. This mirror exists for air-gapped and
    restricted-network environments that cannot reach GitHub. It does not
    track upstream updates -- check the source above before relying on it.

    Context for VCF planning: docs/05-day2-deployments.md, section D
...
$VCFAutomationNetworkMoRef = "dvportgroup-58" # Use PowerCLI to retrieve (e.g. Get-VirtualPortGroup -Name DVPG_FOR_FLEET_MANAGEMENT)
$VCFAutomationNetworkGatewayCidr = "172.30.70.1/24"
...
$payload = @{
    componentSpecs = @(
        [ordered]@{
            ...
            sddcLcmId = $sddcLcmId
            fqdn = $VCFAutomationFQDN
            version = "9.1.0.0"
            configSpec = @{
                size = $VCFAutomationSize
                adminSystemPassword = $VCFAutomationServicesPassword
            }
            vspClusterSpec = [ordered]@{
                deploymentType = "VspClusterSpec"
                sddcLcmId = $sddcLcmId
                platformFqdn = $VCFAutomationRuntimeFQDN
                systemUserPassword = $VCFAutomationServicesPassword
                size = $VCFAutomationSize
                ipv4Pool = @{
                    "addresses" = $VCFAutomationServicesIps
                }
                networkMoId = $VCFAutomationNetworkMoRef
                gatewayCidrIpv4 = $VCFAutomationNetworkGatewayCidr
            }
        }
    )
}
...
try {
    My-Logger "Starting VCF Automation (VCFA) Deployment Validation ..."
...
if($ValidateOnly -eq $false) {
    My-Logger "Starting VCF Automation Deployment to alternative network ${VCFManagementServicesNetworkName} ..."
    $connectivityParams = @{
