...
| `tools/Get-VCFCredentials.ps1`      | Read-only retrieval of the credentials SDDC Manager stores/rotates for its managed components (ESXi, vCenter, NSX, PSC, backup) via `GET /v1/credentials` — filters by resource/account/credential type; passwords **masked by default** (`-ShowPasswords`, `-ExportCsv`) |
