...
| `docs/26-vcf-operations-vvf-vcenter.md` | Build guide — VCF Operations in VVF/standalone: creating the vCenter role and service account by hand (no Fleet LCM), assigning the permission, and adding the vCenter adapter instance in VCF Operations |
...
| `tools/Get-VCFDeploymentArtifacts.ps1` | Read-only capture of re-submittable spec JSON from a built instance — bring-up spec, every Fleet LCM `VSP` component + config, domain / cluster specs, a curated set of NSX policy scopes, Supervisor, vCenter config profiles. Sanitises on write (secrets redacted, licence keys masked, NSX realised-state stripped); `-Tokenize` (FQDN/IP/CIDR → placeholders + `token-map.json`), `-Raw`, `-Include`, `-WhatIf`. See `docs/21-config-artifacts.md` |
