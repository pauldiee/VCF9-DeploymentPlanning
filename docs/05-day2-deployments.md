...
| **Identity Broker**           | One appliance                                                  | **Additional instance only** — the first broker is deployed **at bring-up** with the VCF Management Services (no opt-in; FQDN + services-runtime IP asked by the Installer). Day-N work on the bring-up broker is configuration: identity provider (AD/LDAP), user/group provisioning |
