...
| **Cloud Proxy** (Ops collector)| One or more collector appliances                              | **Additional collectors only** — a unified cloud proxy is configured **by default at bring-up** by the VCF Installer. Stays on the VLAN / VM-mgmt side (`localRegion`) even for NSX-overlay placement. **Friendly Name** on the Add wizard is just a display label — TechDocs doesn't document the field, keep it consistent with your FQDN naming. **Set the Docker Subnet CIDR explicitly** — see the field note in [`27-vcf-operations-ha-cloud-proxy-vvf.md`](27-vcf-operations-ha-cloud-proxy-vvf.md#field-notes). **Root password complexity — see B.4**. **Field-verified 2026-09-15: expect up to ~15 minutes** between the Cloud Proxy showing connected/"going online" in VCF Operations and reaching fully online — normal registration delay, not stuck; only chase the Docker Subnet overlap ([`27-vcf-operations-ha-cloud-proxy-vvf.md`](27-vcf-operations-ha-cloud-proxy-vvf.md#field-notes)) if it's still not online well past that |
...
Operations.** You create the role and service account in vCenter yourself,
then add the vCenter adapter instance in VCF Operations — full step-by-step
in [`26-vcf-operations-vvf-vcenter.md`](26-vcf-operations-vvf-vcenter.md).
...
removing VCF Automation cleanly and deploying it fresh. That procedure — the
`cleanup_component.py` sequence, the `VCFMS_USERNAME`/`VCFMS_PASSWORD`
