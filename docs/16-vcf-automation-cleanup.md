...
`vsp-component`, the last uses `vsp-cluster`. Fill in each `<placeholder>`
with the literal value and **double-quote every one of them**, even the
ones that look safe — a password or FQDN containing a special character
(`$`, spaces, `&`) silently mangles the argument rather than erroring, the
same class of trap as the `$`-interpolation lockout documented elsewhere in
this repo.
