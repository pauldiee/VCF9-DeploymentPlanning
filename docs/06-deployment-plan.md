...
- **Story 6.4 — Backup & lifecycle.** Configure SFTP backups — including **each vCenter's file-based backup, set manually in that vCenter's management interface (VAMI)**; VCF does not configure it for you (see [`08-backup-target.md`](08-backup-target.md) §A). Connect the depot for **fleet lifecycle** (SDDC Manager already has its own depot from bring-up — this is the fleet-wide LCM depot, not a re-do). Ref: [File-Based Backups for SDDC Manager, NSX Manager and vCenter](https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-0/fleet-management/backup-and-restore-of-cloud-foundation/file-based-backups-for-sddc-manager-and-vcenter-server.html).
