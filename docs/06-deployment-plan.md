...
- **Story 8.3 — Optional fleet components.** Deploy the remaining fleet components as needed: **Log Management**, VCF Operations for Networks, and the Identity Broker. Each is individually selectable in the [export tool](https://pauldiee.github.io/VCF9-DeploymentPlanning/tools/deployment-plan/) — the generated story lists only the selected ones (and if the Identity Broker is out of scope, E6 6.3 becomes the identity path and 8.4 drops the broker-based fleet SSO).
