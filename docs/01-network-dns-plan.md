...
| License Hub (optional)          | ~9 (+**1 FQDN**) |       | The **SSP Installer needs a real FQDN** (its OVA requires one — *"must contain a dot character"*), so plan **A + PTR** for it, not just an IP. Only if **vDefend or Avi** is in scope — the licensing appliance the **SSP Installer** deploys, and **not** the `License Server` two rows up (they coexist). One subnet, two IP pools: **installer 1**, **controller + worker nodes 4**, **License Hub services 4**. The node and service pools **cannot be changed after deployment** — size for scale-out now. See `prerequisites.md` |
