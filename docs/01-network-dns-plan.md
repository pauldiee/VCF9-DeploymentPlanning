...
| License Hub (optional)          | ~9 (+**3 FQDNs**) |       | **Three FQDNs, not one.** The **SSP Installer** OVA requires one (*"must contain a dot character"*), and the **License Hub instance** it deploys adds an **Instance FQDN** (required) and a **Messaging FQDN** (mandatory status unconfirmed — plan it). **A + PTR** for each. Only if **vDefend or Avi** is in scope — and **not** the `License Server` two rows up (they coexist). One subnet, two IP pools: **installer 1**, **controller + worker nodes 4**, **License Hub services 4** — entered as **contiguous start–end ranges**, so reserve an **unbroken block**. The node and service pools **cannot be changed after deployment** — size for scale-out now. Needs a **distributed** port group. See `prerequisites.md` |
