...
| Avi Controller cluster FQDN (optional) | `sfo-m01-avi01.sfo.example.io` | VM Mgmt subnet — the 3 controller nodes are **IP-only** (no DNS records). **Must resolve BEFORE the deploy**: the wizard has **no VIP field**, so it takes the VIP from this record. Also decide a separate **Cluster Name** |
