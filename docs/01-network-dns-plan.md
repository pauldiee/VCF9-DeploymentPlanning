...
| VCF Operations for Networks (optional) | 2 (+2 if Large) |  | Platform node + collector node — lands here when the Day-2 placement is the **Shared Management Network** (a **Large** platform is a 3-node cluster: +2). **IP-only: the deploy wizard has no FQDN field, so A/PTR records are optional, not required** — create them anyway for operational sanity (`05-day2-deployments.md` B.2) |
