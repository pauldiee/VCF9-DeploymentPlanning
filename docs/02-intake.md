...
|E14| VCF fleet/services FQDNs new in 9.x — Cloud Proxy, License Server, Identity Broker, VCF services runtime (the **fleet** management-services runtime — **not** Automation's same-named runtime in `E10`), Ops for Networks platform + collector (each needs A+PTR+IP; services-runtime / fleet-services FQDNs **lowercase only** — TechDocs); several may be Day-2 → `05-day2-deployments.md` | `[MGMT]`  |
