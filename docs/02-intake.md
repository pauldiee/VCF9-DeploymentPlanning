...
|H5 | Enable **vSphere Supervisor**? (requires centralized edge gateway; needs Service CIDR + control-plane IP range — **5 consecutive IPs**: 3 nodes + floating + upgrade spare — plus an **API FQDN** with a DNS record). If yes: **load-balancer choice** — built-in NSX/VPC LB / Foundation Load Balancer / **Avi** (Avi → `E16`/`F11`, controller cluster **before activation**). Full checklist incl. per-networking-path IP blocks (9.1: VPC transit-gateway block must be a `/16`): `prerequisites.md` → vSphere Supervisor | `[WLD]` |
