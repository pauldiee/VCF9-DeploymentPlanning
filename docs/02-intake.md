...
| **License Hub instance** (deploy wizard) `admin` / `audit` | **15–128** | 1 upper, 1 lower, 1 digit, 1 special — **none** of the dictionary / palindrome / monotonic rules above, but a **higher minimum**. A different layer of the same product, so **a password that passes the OVA can still be rejected here**. Only if vDefend/Avi is in scope |
