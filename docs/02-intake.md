...
|E16a| **Does the site ALREADY run Avi, on an older version with legacy (25-character / YAML) licences?** If yes, this is a **pre-upgrade gate, not a follow-up**: upgrading to **32.1.1** gives legacy licences a **strict 90-day grace period from initial boot / upgrade completion**, and that limit **overrides any existing validity dates** — paid-up licences valid for years still stop after 90 days. Migrate the entitlement on the **Broadcom Support Portal** *before* upgrading (one-way: an upgraded licence cannot be downgraded), and decide **Cloud Licensing vs On-prem License Hub** at the same time → `prerequisites.md` | Prereq    |
|E16| **Avi Load Balancer** (only if Avi is the chosen LB — e.g. for Supervisor, optionally in front of VCF Automation, or tenant LB; Supervisor also runs without Avi, and Automation's built-in LB serves its cluster VIP without one): controller size (Small / Large / XLarge), then **per controller set** — 3 controller node **IPs** + cluster VIP IP + **one cluster FQDN** (VM Mgmt subnet, in the **management domain**; only the cluster FQDN needs A+PTR — the workbook's Avi section asks node IPs only) **plus a separate Cluster Name**. **Reserve four IPs but type three** — the wizard takes no VIP address, reaching the VIP via the cluster FQDN, so that record **must resolve before the deploy** (field-confirmed): put it in the Step 1 DNS request, not the post-deploy list. The deploy also asks for the **VCF Ops admin password** alongside the new Avi `admin` one (min 15, specials `!@#$%^&*()~`), and needs the Avi bundle **already in the depot**. Then the controller's **own first-login wizard** (System Settings → Email/SMTP → Multi-Tenant) asks for a **third secret — a Passphrase, which is restore-critical** (it protects the controller's config backups; treat it like the backup encryption passphrase in `F10` — named owner, password manager, lost = no restore) — plus **DNS resolver(s)**, a **DNS search domain**, an **SMTP choice** (defaults to **None**, so nothing alerts anyone) and the **multi-tenancy model** (SEs provider-shared vs per-tenant, shared vs per-tenant IP route domain) — an architecture decision best made **before** first login. None of it is collected by the VCF deploy. **Ask this once per NSX instance, not per WLD** — WLDs sharing an NSX instance share one set; a WLD with its own NSX needs its own. Controllers are **always** in the mgmt domain; **Service Engines** are per cluster in the WLD (min 2) → `prerequisites.md` | Prereq    |
|E17| **License Hub** (only if **vDefend or Avi** is in scope — deployed from the **SSP Installer**; **not** the bring-up `License Server` in `E14`, they coexist): **~9 IPs on one subnet** in two pools (installer 1, controller+worker nodes 4, services 4 — entered as **contiguous ranges**, so reserve an unbroken block; **pools cannot change after deployment**, size for scale-out) and **3 FQDNs** (installer + instance + messaging, A+PTR each) — the **instance and messaging FQDNs must map to the 1st and 2nd IPs of the service pool**, so settle the ranges before requesting DNS. Connecting it to vCenter needs an **administrator credential + the vSphere root CA certificate in hand** (pasted or uploaded — there is no thumbprint prompt), and **no vCenter object name** in the path (datacenter / cluster / datastore / resource pool / storage policy / DVS / port group) may contain `/ , ' = [ ] & % \ "`. Needs a **distributed** port group, a **content-library datastore** and a **non-encrypted** storage policy (encrypted policies are **not supported**, and the policy is immutable); resources are reserved by default (*"required for a production environment"*). **Licensing is a six-step chain, not a deploy**: deploy → register → **load the licence file** → onboard each endpoint (needs that endpoint's **admin credential + certificate**) → assign → **switch the endpoint to On-prem License Hub**. Stopping at "the hub is up" leaves a healthy but **unlicensed** fleet — give the whole chain one owner. Plus **connected or disconnected** mode — chosen at **first login**, not at deploy (it can be skipped and deferred). Both modes need a **Broadcom customer account**: name who holds it. **Connected** needs outbound 443 to **`portal.pulse.broadcom.com`** (Avi Cloud Console — **not** on Broadcom's Public URLs list; it has its own proxy setting). **Disconnected** = air-gapped: a manual registration-file → activation-file → **licence-file** exchange, plus a recurring report-upload/licence-import loop (the **six-monthly import**) — name the owner → `prerequisites.md` | Prereq    |
...
| **SSP Installer appliance** (OVA) `sysadmin` / `admin` / `audit` (+ GRUB root) | min 12 | 1 upper, 1 lower, 1 digit, 1 special, **≥5 different characters, no dictionary words, no palindromes, no monotonic run >4**. Validated at **VM boot**, not in the wizard — a weak password deploys and then forces a change at first login. Only if vDefend/Avi is in scope |
| **License Hub instance** (deploy wizard) `admin` / `audit` | **15–128** (TechDocs says 12 — **the product enforces 15**) | 1 upper, 1 lower, 1 digit, 1 special — **none** of the dictionary / palindrome / monotonic rules above, but a **higher minimum**. A different layer of the same product, so **a password that passes the OVA can still be rejected here**. Only if vDefend/Avi is in scope |
